...
app.get("/api/diagnostics", async (req, res) => {
  ...
  // ── MODE 0d: ?apply=macma-cena-setup ── vytvorí "Macma - Onix (cena)" config
  // (pricelist feed, price×1.2 → Default_Price, len aktualizácia – onMissing=skip, bez H kódov)
  if (apply === "macma-cena-setup") {
    try {
      const NEW_NAME = "Macma - Onix (cena)";
      const allConfigs = await storage.getAllSyncConfigs();
      const existing: any = (allConfigs as any[]).find((c: any) => c.name === NEW_NAME);
      if (existing) {
        return res.json({
          _mode: "apply", apply: "macma-cena-setup",
      ...
      // Pricelist matchNormalization – rovnaká ako SKU config
      ...
      const created: any = await storage.createSyncConfig({
        ...
        schedule: { enabled: false, frequency: "daily", timeOfDay: "06:00", backupBeforeSync: false },
        notes: "Aktualizácia ceny z MACMA pricelist feedu. Koeficient 1.2 na poli price → Default_Price. Nevytvára nové karty.",
        isEnabled: true,
        autoRetry: false,
        retryDelayMin: 3,
      } as any);
      ...
      await storage.createAuditLog({
        ...
        details: { apply: "macma-cena-setup", sourceModule: "MACMA", sourceDataSource: "pricelist" },
      } as any);
