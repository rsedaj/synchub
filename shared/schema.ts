...
export const syncConfigs = pgTable("sync_configs", {
  ...
  matchOperator: text("match_operator").default("and"),
  matchNormalization: jsonb("match_normalization").$type<{ caseInsensitive?: boolean; collapseWhitespace?: boolean; stripLeadingZeros?: boolean; normalizeDecimals?: boolean; stripDiacritics?: boolean } | null>(),
...
export const insertSyncConfigSchema = createInsertSchema(syncConfigs).pick({
  ...
  targetModuleId: true,
  sourceModuleId: true,
  targetDataSource: true,
  sourceDataSource: true,
  sourceRecordLimit: true,
  fieldMappings: true,
  matchFields: true,
  matchOperator: true,
  matchNormalization: true,
  onMissing: true,
  targetStock: true,
  sourceFilters: true,
  hKodConfig: true,
