                        ...
                        {editor.hKodConfig.enabled && (
                          <div className="ml-7 space-y-4 border-l-2 border-muted pl-4">
                            ...
                            <div className="bg-muted/40 border rounded p-2.5 text-[11px] text-muted-foreground space-y-0.5">
                              ...
                              <p>2. {language === "sk" ? `ONIX záznam nájdený, Ns_Number NEZAČÍNA "${editor.hKodConfig.detectionPrefix || editor.hKodConfig.prefix || "H20"}" → priradí sa nový H kód. Ak bol H kód priradený v predchádzajúcom rune, znovu sa použije rovnaký (ochrana pred duplikátmi).` : `ONIX record found, Ns_Number does NOT start with "${editor.hKodConfig.detectionPrefix || editor.hKodConfig.prefix || "H20"}" → new H code assigned. If already assigned in a previous run, the same code is reused (anti-duplicate).`}</p>
