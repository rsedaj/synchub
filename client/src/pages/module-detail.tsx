...
const MODULE_HELP: Record<string, { description: string; apiInfo: string; endpoints?: string[]; authInfo: string; dataFields: string; notes?: string; links?: { label: string; url: string }[] }> = {
  ...
  ANDA: {
    description: "Dodávateľ reklamných predmetov Anda Present Ltd. XML a CSV feedy generované individuálne s vlastnými zľavovými cenami. Prístup cez unikátne Feed ID + IP whitelist.",
    apiInfo: "XML/CSV feedy v2.8 na xml.andapresent.com. 9 typov feedov: 7 XML (produkty, ceny, sklady, potlač, kategórie, cenník potlače, unikátne ceny) + 2 CSV (produkty, ceny). Sklady aktualizované každých 30 min.",
    endpoints: [
      "/export/products/{lang}/{feedId}",
      "/export/prices/{feedId}",
      "/export/inventories/{feedId}",
      "/export/labeling/{lang}/{feedId}",
      "/export/categories/{lang}/{feedId}",
      "/export/labeling-prices/{feedId}",
      "/export/unique-product-prices/{feedId}",
      "/export/products-csv/{lang}/{csvId}",
      "/export/prices-csv/{csvId}",
    ],
    authInfo: "Unikátne Feed ID (XML) a CSV ID — poskytne Anda po aktivácii. Vyžaduje statickú IP adresu na whitelist. Kontaktujte svojho obchodného zástupcu.",
    dataFields: "Item Number, Design Name, Primary/Secondary Color, Name, Description, Images, MOQ, Weight, Country of Origin, Tariff Number, Brand, EAN Code, Prices (listPrice/discountPrice), Stock (central/external/incoming), Labeling (techniky, pozície, plochy), Categories, Packaging, Specification (materiál, veľkosť, kapacita)",
    notes: "Frekvencia aktualizácie: Produkty 3x/týždeň, Ceny 1x/týždeň (So 22:00), Sklady každých 30 min, Potlač 3x/týždeň, Kategórie 2x/deň.\nDostupné jazyky (19): sk, cz, en, de, hu, it, fr, nl, pl, ro, no, se, dk, fi, gr, si, bg, es, pt.\nCSV feedy majú odlišné ID od XML feedov.\nAktivačné požiadavky: 1) Kontaktovať sales rep, 2) Poskytnúť statickú IP adresu.",
    links: [
