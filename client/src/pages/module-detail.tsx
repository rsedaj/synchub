...
const MODULE_HELP: Record<string, { description: string; apiInfo: string; endpoints?: string[]; authInfo: string; dataFields: string; notes?: string; links?: { label: string; url: string }[] }> = {
  ...
  PFCONCEPT: {
    description: "Dodávateľ reklamných predmetov PF Concept International B.V. (Holandsko). Jeden z TOP 3 najväčších európskych dodávateľov promo produktov s portfóliom 5 značiek: Bullet (štandard), Avenue (premium), Seasons (sezónne), US Basic (textil), Elevate Life (lifestyle). Katalóg: ~5 000+ produktov.\n\nDáta sú dostupné cez Data Feeds Gateway v3 — XML/JSON feedy na adrese www.pfconcept.com/portal/datafeed/. SyncHub používa 4 XML feedy: Product Feed (CZ jazyk), Price Feed (individuálny cenník), Print Price Feed (ceny potlače), Stock Feed (skladové zásoby 2× denne).\n\nSklad: centrálny v Holandsku, distribúcia do celej Európy. Obrázky: images.pfconcept.com (live server).",
    apiInfo: "Data Feeds Gateway v3 na www.pfconcept.com/portal/datafeed/\n\n4 aktívne feedy v SyncHub:\n\n1. Product Feed v3 (CZ) — kompletný katalóg\n   • URL: productfeed_cz_v3.xml\n   • Aktualizácia: 1× denne o 02:30\n   • Obsah: modely → items (modelCode, itemCode, popis, rozmery, hmotnosť, farby, materiál, EAN, brand, kategórie, obrázky, potlačové info)\n   • 70+ dátových polí na produkt\n\n2. Price Feed v3 — individuálny cenník\n   • URL: pricefeed_{unique_code}_v3.xml\n   • Aktualizácia: 1× týždenne (pondelok 05:00)\n   • Obsah: modelCode → items (itemCode, currency, nettPrice, 4 cenové stupne/scales, minDecoQty, clearance, promo ceny)\n\n3. Print Price Feed v3 — ceny potlačových techník\n   • URL: printpricefeed_{unique_code}_v3.xml\n   • Aktualizácia: 1× týždenne (pondelok 05:00)\n   • Obsah: printCode, impMethod (pad print, screen, laser...), LTMCharge, logoSizes, cenové stupne podľa počtu farieb a množstva\n\n4. Stock Feed v3 — skladové zásoby\n   • URL: stockfeed_{unique_code}_v3.xml\n   • Aktualizácia: 2× denne (01:30, 12:30)\n   • Obsah: modelCode → items (itemCode, qty, freeQty, nextDeliveryDate/Qty)\n\nImage server: http://images.pfconcept.com/ (live linky na obrázky)",
    endpoints: [
      "Product Feed v3 — productfeed_cz_v3.xml (katalóg, denne 02:30)",
      "Price Feed v3 — pricefeed_{code}_v3.xml (cenník, pondelok 05:00)",
      "Print Price Feed v3 — printpricefeed_{code}_v3.xml (potlač, pondelok 05:00)",
      "Stock Feed v3 — stockfeed_{code}_v3.xml (sklady, 2× denne)",
      "Image Server — images.pfconcept.com (live)",
    ],
    authInfo: "Username/Password pre Data Feeds Gateway portál.\n\nAktuálne prístupy (uložené vo Vault):\n• Username: production@hauerland.sk ✅\n• Heslo: uložené v env PFCONCEPT_PASSWORD ✅\n\nPortál: www.pfconcept.com/cs_cz/data-feeds-gateway\n\nPriame URL feedy nevyžadujú autentifikáciu — obsahujú unikátny kód (UUID) v URL adrese, ktorý slúži ako identifikátor partnera.\n\nVaše unikátne kódy:\n• Price + Print Price + Stock: ff7e834a_8a5f_39a8_4014_b6f9c8ca81aa\n• Product Feed: verejný (productfeed_cz = český jazyk)\n\nDôležité: URL adresy feedov sú viazané na partnerskú firmu a určujú cenové podmienky.",
    dataFields: "Product Feed (70+ polí na model/item):\n• model: modelCode, description, extDesc, keywords\n• item: itemCode, size, sizeGrid, gender, material, eanCode, brand, countryOfOrigin, hsCode, launchDate\n• measurements: weightGr, grammage, lengthCm, heightCm, widthCm, diameterCm\n• colors: colorCode, colorDesc, baseColor, hexColor, pmsColorReference\n• categoryData: groupCode, groupDesc, catCode, catDesc\n• decorationSettings: method, leadTime, impLocationDefault, impWidth/HeightDefaultMm, maxColoursDefault\n• packaging: decoPackagingIndiv, decoPackaging, grossWeightKg, nettWeightKg, qtyPerCarton\n• images: imageMain, imageFront, imageBack, imageLogoY1-3, imageExtra1-3, imageDetail1-3, imageMood1-3, imagePackage, imageGroup, imageModel, videoUrl1-2\n• catalogues: year, page\n• battery: batteryNr, batterySize, batteryType\n\nPrice Feed:\n• itemCode, currency (EUR), clearance, decoCharge, minDecoQty\n• scales (4 stupne): priceBar (od množstva), nettPrice, type, industryCol\n• customerSpecificPrice: startDate, endDate (promo)\n\nPrint Price Feed:\n• printCode, impMethod, LTMCharge, priceDependence\n• logoSizes → amountColors → amountSetupCharges → decoPrices (od množstva)\n\nStock Feed:\n• itemCode, qty (celkové), freeQty (dostupné), nextDeliveryDate, nextDeliveryQty",
    notes: "STAV: ✅ PRIPOJENÉ — 4 XML feedy aktívne (Product, Price, Print Price, Stock)\n\nŠtatistiky feedov:\n• Product Feed: ~5 000+ modelov s desiatkami itemov (farieb/veľkostí)\n• Price Feed: individuálne ceny pre Hauerland (4 cenové stupne)\n• Print Price Feed: ceny všetkých potlačových techník\n• Stock Feed: real-time zásoby (aktualizácia 2× denne)\n\nVerzionácia: v3 (máj 2020) — podporuje XML aj JSON formát\nJazyk: CZ (čeština) — produktové popisy v češtine\n\nZnačky PF Concept:\n• Bullet — štandardná promo línia\n• Avenue — premium produkty\n• Seasons — sezónne kolekcie\n• US Basic — textilná línia\n• Elevate Life — lifestyle produkty\n\nObrázky: Live server images.pfconcept.com\n• Typy: imageMain, imageFront, imageBack, imageLogoY, imageExtra, imageDetail, imageMood, imagePackage, imageGroup, imageModel\n• Formát: priame URL, viacero uhlov a veľkostí\n\nSynchronizačné scenáre:\n• Product Feed → ONIX skladové karty (import katalógu, obrázky, rozmery, EAN)\n• Price Feed → ONIX nákupné cenníky (4 cenové stupne)\n• Stock Feed → real-time kontrola dostupnosti pred objednávkou\n• Print Price Feed → kalkulácie potlače pre e-shop (TronShop)\n• Images → e-shop Promotron/TronShop (priame URL z images.pfconcept.com)",
    links: [
      { label: "PF Concept Data Feeds Gateway", url: "https://www.pfconcept.com/cs_cz/data-feeds-gateway" },
      { label: "PF Concept E-shop", url: "https://www.pfconcept.com" },
      { label: "PF Concept Partner Login", url: "https://www.pfconcept.com/login" },
      { label: "PF Concept Image Server", url: "http://images.pfconcept.com/" },
