...
const MODULE_HELP: Record<string, { description: string; apiInfo: string; endpoints?: string[]; authInfo: string; dataFields: string; notes?: string; links?: { label: string; url: string }[] }> = {
  ...
  MID: {
    description: "Dodávateľ reklamných predmetov Midocean Brands (Holandsko). Jeden z najväčších európskych dodávateľov s portfóliom značiek: midocean (promo), Elevate (premium), Bloom (eko), Kote (outdoor).\n\nAPI poskytuje prístup k produktovému katalógu s cenami, obrázkami, špecifikáciami a aktuálnymi skladovými zásobami. Dáta sú dostupné v 25+ jazykoch.",
    apiInfo: "REST API v2.0 — JSON formát.\nBase URL: https://api.midocean.com/2.0\n\nHlavné endpointy:\n• Products — kompletný katalóg (~3 000 produktov), kategórie, varianty\n• Prices — nákupné ceny (debtor-specific), katalógové ceny, EDP ceny\n• Stock — aktuálne skladové zásoby v reálnom čase (centrálny sklad NL)\n• Print data — potlačové techniky, pozície, plochy, šablóny\n• Media — obrázky produktov (rôzne uhly, lifestyle, hi-res)\n\nDáta sa dajú filtrovať podľa značky, kategórie, farby, novosti a ďalších parametrov.",
    endpoints: [
      "GET /2.0/products — kompletný katalóg produktov",
      "GET /2.0/products/{masterCode} — detail produktu",
      "GET /2.0/stock — aktuálne skladové zásoby",
      "GET /2.0/stock/{masterCode} — sklad konkrétneho produktu",
      "GET /2.0/prices — cenníky (nákupné ceny)",
      "GET /2.0/print — potlačové dáta (techniky, pozície, plochy)",
      "GET /2.0/media — obrázky a médiá produktov",
    ],
    authInfo: "API Key autentifikácia — kľúč sa posiela v hlavičke x-Gateway-APIKey.\n\nAPI kľúč sa získava z Midocean Partnerskej zóny (partner portal) alebo od obchodného zástupcu.\n\nDôležité: API kľúč je viazaný na partnerskú firmu a určuje aj cenové podmienky.",
    dataFields: "Products: masterCode, variantCode, názov (lokalizovaný), krátky/dlhý popis, materiál, rozmery (dĺžka/šírka/výška), hmotnosť (netto/brutto), objem, farba (master/variant), kategória, podkategória, brand, krajina pôvodu, HS kód, minimálne objednávacie množstvo, balenie (inner/master carton)\nPrices: nákupná cena, doporučená predajná cena, EDP cena, mena, cenové skupiny\nStock: aktuálny stav, prvý dostupný dátum, očakávané dodávky\nPrint: technika, pozícia, rozmery plochy, max farby, nastavenie, šablóna\nMedia: product images (hlavný, uhly, lifestyle), hi-res, logo templates",
    notes: "Dôležité informácie:\n\n• Server občas vracia 503 (údržba) — SyncHub automaticky retryuje\n• Obrázky: viacero uhlov + lifestyle fotky vo vysokom rozlíšení\n• Značky: midocean (štandard), Elevate (premium), Bloom (eko/udržateľné), Kote (outdoor)\n• Cenníky: nákupné ceny sú individuálne podľa partnerskej zmluvy\n• Sklad: centrálny sklad v Holandsku, aktualizácia niekoľkokrát denne\n\nSynchronizačné scenáre:\n• Produkty → ONIX skladové karty (import katalógu)\n• Ceny → ONIX nákupné cenníky\n• Sklady → kontrola dostupnosti (pred objednávkou)\n• Obrázky → e-shop (automatický import do TronShop)\n\nPre získanie API kľúča kontaktujte svojho obchodného zástupcu Midocean.",
    links: [
      { label: "Midocean Partner Portal", url: "https://www.midocean.com" },
      { label: "Midocean API Dokumentácia", url: "https://api.midocean.com/2.0/docs" },
    ],
  },
  STICKER: {
