...
const MODULE_HELP: Record<string, { description: string; apiInfo: string; endpoints?: string[]; authInfo: string; dataFields: string; notes?: string; links?: { label: string; url: string }[] }> = {
  ...
  MID: {
    description: "Dodávateľ reklamných predmetov Midocean Brands B.V. (Holandsko). Jeden z najväčších európskych dodávateľov promo produktov.\n\nDva spôsoby integrácie:\n1. REST API — live dotazy s JSON/XML výstupom, zákaznícky špecifické ceny a objednávky\n2. SFTP — súbory na stiahnutie (XML v1.0/v1.1, CSV v2.0) na transfer.midocean.com\n\nSyncHub používa REST API pre real-time prístup k produktom, skladom, cenníkom, potlačovým dátam a objednávkam.\n\nDva prostredia:\n• Production: api.midocean.com (reálne dáta, reálne objednávky)\n• Test: apitest.midocean.com (na testovanie, vypnuté mimo pracovných hodín v EÚ)",
    apiInfo: "REST API v2.0 — Base URL: https://api.midocean.com\nDokumentácia: v1.10.1 (02-11-2023)\n\n5 informačných endpointov + 2 servisné:\n\n1. Products v2.0 — kompletný katalóg vrátane textilu\n   • Popis, obrázky, atribúty, varianty, kategórie, certifikáty\n   • 15 jazykov: EN, DE, ES, FR, IT, HU, NL, PL, PT, RO, RU, SV, NO, CS, DA, FI\n   • Parameter ?language=sk pre jazyk\n   • Aktualizácia: denne (v noci)\n\n2. Stock v2.0 — aktuálne skladové zásoby\n   • SKU, aktuálny stav, najbližšie 2 dodávky (dátum + množstvo)\n   • Agregát skladov NL + PL\n   • Aktualizácia: každú hodinu\n\n3. Pricelist v2.0 — zákaznícky špecifické ceny\n   • Ceny podľa vašej partnerskej zmluvy\n   • Množstevné stupne (scales)\n   • Aktualizácia: denne\n\n4. Print Data v1.0 — potlačové informácie\n   • Pozície, techniky, maximálne veľkosti, max farby, šablóny\n   • Preklady názvov techník v 15 jazykoch\n   • Aktualizácia: denne\n\n5. Print Pricelist v2.0 — ceny potlače\n   • Cenníky podľa techniky a manipulačnej skupiny (A-E, Z)\n   • 5 typov výpočtu: NumberOfPositions, NumberOfColours, Area, AreaRange, ColourAreaRange\n   • Aktualizácia: denne\n\nServisné endpointy:\n6. Order Entry v2.0 — vytváranie objednávok (NORMAL, PRINT, SAMPLE)\n7. Order Details v2.0 — detail a tracking existujúcich objednávok\n\nFormáty výstupu: JSON (predvolený), XML (header Accept: text/xml), CSV (niektoré endpointy)",
    endpoints: [
      "GET /gateway/products/2.0?language={lang} — produktový katalóg (JSON/XML)",
      "GET /gateway/stock/2.0 — skladové zásoby, aktualizácia každú hodinu",
      "GET /gateway/pricelist/2.0 — zákaznícky cenník (individuálne ceny)",
      "GET /gateway/printdata/1.0 — potlačové dáta (pozície, techniky, plochy)",
      "GET /gateway/printpricelist/2.0 — cenník potlače (JSON/XML/CSV)",
      "POST /gateway/order/2.0/create — vytvorenie objednávky (NORMAL/PRINT/SAMPLE)",
      "GET /gateway/order/2.0/detail?order_number={num} — detail a tracking objednávky",
    ],
    authInfo: "API Key autentifikácia — kľúč sa posiela v HTTP hlavičke:\n   Key: x-Gateway-APIKey\n   Value: váš-api-kľúč\n\nZískanie API kľúča:\n1. Registrovať sa na www.midocean.com (B2B účet)\n2. Prihlásiť sa → Account → Customer API tab\n3. Váš REST API Key sa zobrazí okamžite\n4. Vybrať, ku ktorým API sa chcete prihlásiť (Subscribe)\n\nDva kľúče:\n• Production key — z webshop účtu (reálne dáta)\n• Test key — vyžiadať od obchodného zástupcu (testovacie prostredie)\n\nDôležité: Ak dostanete chybu 'Unauthorized Application Request', overte:\n• Správne napísaný kľúč v hlavičke x-Gateway-APIKey\n• Aktivované subscriptions pre požadované API\n• Ak kľúč nefunguje: Account → Customer API → Request new API key",
    dataFields: "Products v2.0: master_code, sku, master_id, variant_id, product_name, plc_status (NEW/COLLECTION/OUTLET), short_description, long_description, dimensions, net_weight, gross_weight, color_code, color_description, color_group, pms_color, size_textile, material, green (eko), printable, brand, category_code, product_class, category_level1/2/3, commodity_code (Intrastat), country_of_origin, gtin (EAN), release_date, digital_assets (obrázky: front, back, ambient, box, print — 700×700 + original hi-res), packaging (inner/carton qty, rozmery, hmotnosť), polybag\n\nStock v2.0: sku, qty (aktuálny stav = NL+PL sklad), first_arrival_date, first_arrival_qty, next_arrival_date, next_arrival_qty\n\nPricelist v2.0: sku, variant_id, price (vaša cena), valid_until, scale[] (minimum_quantity + price = množstevné stupne)\n\nPrint Data v1.0: master_code, master_id, item_color_numbers, print_manipulation (A/B/C/D/E/Z), printing_positions[] (position_id, max_print_size_height/width, printing_techniques[] s id a max_colours), print_template (PDF URL)\n\nPrint Pricelist v2.0: currency, validity, print_manipulations[] (code, description, price), print_techniques[] (id, description, pricing_type, setup, setup_repeat, var_costs[] so scales)\n\nOrder: po_number, order_type (NORMAL/PRINT/SAMPLE), sku/master_code, quantity, shipping_address, printing_positions[], order_number (response), total_item_price, total_print_costs, freight_charge, tracking",
    notes: "SFTP prístup (doplnkový):\n• Server: sftp://transfer.midocean.com\n• Súbory: stock.xml, stock_textile.xml, prodinfo_{lang}.xml (v1.0/v1.1), prodinfo_{lang}.csv (v2.0), printinfo.xml, USBprodinfo.xml, USBpricelist.xml\n• SFTP účet sa žiada od obchodného zástupcu\n\nFrekvencia aktualizácie REST API:\n• Stock: každú hodinu\n• Products: denne (v noci)\n• Pricelist: denne\n• Print Data: denne\n• Print Pricelist: denne\n\nObjednávky cez API:\n• NORMAL — nepotlačený tovar, bez limitu množstva (okrem textilu!)\n• PRINT — s potlačou, konfigurácia pozícií a farieb\n• SAMPLE — vzorky, max 5 ks na riadok (aj textil)\n• Max 50 položiek na objednávku\n• po_number musí byť unikátne (API blokuje duplikáty)\n• Po odoslaní sa dá zrušiť len cez Customer Care\n\nTypy výpočtu potlačových cien:\n• NumberOfPositions — cena × počet pozícií × množstvo\n• NumberOfColours — cena × počet farieb × množstvo\n• Area — cena × plocha (cm²) × množstvo\n• AreaRange — cena podľa rozsahu plochy × množstvo\n• ColourAreaRange — kombinácia plochy a farieb\n\nCDN obrázky: cdn1.midocean.com/image/{size}/{sku}.jpg\n• Veľkosti: 700X700 (štandard), 170X170 (thumbnail), original (hi-res)\n• Typy: front, back, ambient, box, print, side\n\nPLC Status produktov:\n• NEW — novinka z posledného katalógu\n• COLLECTION — štandardná kolekcia\n• OUTLET — vypredaj, vyradený po dopredaní\n\nChybové kódy:\n• 503 — služba nedostupná (test server mimo pracovných hodín)\n• Unauthorized — nesprávny kľúč alebo chýbajúca subscription\n• 999 (order) — chyba objednávky (insufficient stock, invalid date, duplicate po_number)\n\nSynchronizačné scenáre:\n• Products → ONIX skladové karty (import katalógu)\n• Pricelist → ONIX nákupné cenníky (zákaznícky špecifické ceny)\n• Stock → kontrola dostupnosti pred objednávkou (každú hodinu)\n• Order Entry → automatické objednávky z ONIX\n• Print Data + Print Pricelist → kalkulácie potlače pre e-shop",
    links: [
      { label: "Midocean Webshop (API Key)", url: "https://www.midocean.com" },
      { label: "Test API Environment", url: "https://apitest.midocean.com" },
      { label: "Postman (testovanie API)", url: "https://www.postman.com" },
  ...
  XDCONNECT: {
    description: "Dodávateľ reklamných predmetov XD Connects (predtým Xindao, Holandsko). Prémiový dodávateľ s dôrazom na udržateľnosť a dizajn. Značky: XD Design (premium), XD Collection (štandard), XD Apparel (textil).\n\nDáta sú dostupné cez Data Feeds portál — XML a CSV súbory na stiahnutie po prihlásení. Feedy obsahujú produktový katalóg, ceny, skladové zásoby, obrázky a potlačové informácie.\n\nPre automatický prístup z SyncHub je potrebné nastaviť priame URL feedov alebo FTP/API prístup.",
    apiInfo: "Data Feeds portál na www.xdconnects.com — prihlasovacie údaje potrebné pre prístup.\n\nDostupné typy feedov:\n• Product Feed — kompletný katalóg produktov (XML/CSV)\n• Price Feed — nákupné cenníky podľa partnerskej zmluvy\n• Stock Feed — aktuálne skladové zásoby (aktualizácia niekoľkokrát denne)\n• Media Feed — URL adresy obrázkov (produktové, lifestyle, hi-res)\n• Print Feed — potlačové techniky, pozície a ceny\n\nFormáty: XML, CSV (závisí od typu feedu)\nKódovanie: UTF-8\nAktualizácia: sklady niekoľkokrát denne, produkty a ceny denne alebo týždenne",
    endpoints: [
      "Product Feed URL — katalóg produktov (XML/CSV)",
      "Price Feed URL — nákupné cenníky",
      "Stock Feed URL — skladové zásoby (aktualizácia niekoľkokrát denne)",
      "Media Feed URL — obrázky produktov a lifestyle fotky",
      "Print Feed URL — potlačové techniky a cenníky",
    ],
    authInfo: "Username/Password autentifikácia pre prístup k Data Feeds portálu.\n\nPrístupové údaje sa získavajú:\n1. Registrácia na www.xdconnects.com (B2B účet)\n2. Kontaktovať customer service pre aktiváciu Data Feeds prístupu\n3. Obdržíte login + heslo + priame URL adresy feedov\n\nNiektoré feedy môžu vyžadovať HTTP Basic Auth (username:password v URL alebo hlavičke).",
    dataFields: "Products: kód produktu, názov (viacjazyčný), krátky/dlhý popis, materiál, rozmery (dĺžka/šírka/výška cm), hmotnosť (g), objem (ml), farba, veľkosť, kategória, podkategória, brand (XD Design/Collection/Apparel), krajina pôvodu, HS kód, EAN, certifikáty (BSCI, GRS, Oeko-Tex)\nPrices: nákupná cena, doporučená predajná cena, mena, cenové skupiny podľa množstva\nStock: aktuálny stav, prvý dostupný dátum, očakávané dodávky (dátum + množstvo), sklad (NL)\nPrint: technika (screen, digital, laser, embroidery, doming, sublimation), pozícia, rozmery plochy, max farby, cena setup, cena za kus podľa množstva\nMedia: hlavný obrázok, detaily, lifestyle, balenie, hi-res URLs",
    notes: "Stav integrácie: ČAKÁ SA NA PRÍSTUPOVÉ ÚDAJE\n\nKroky na aktiváciu:\n1. Kontaktovať XD Connects customer service alebo obchodného zástupcu\n2. Vyžiadať prístup k Data Feeds (XML/CSV)\n3. Získať priame URL adresy feedov + prihlasovacie údaje\n4. Nakonfigurovať v SyncHub: Feed URL + Username + Password\n5. Otestovať pripojenie a Data Preview\n\nDôležité informácie:\n• XD Connects kladie dôraz na udržateľnosť — mnoho produktov má eko certifikáty\n• Sklady sú v centrálnom sklade v Holandsku\n• Minimálne objednávacie množstvo: obvykle od 25 ks\n• Obrázky: vysoké rozlíšenie, viacero uhlov + lifestyle fotky\n\nSynchronizačné scenáre:\n• Product Feed → ONIX skladové karty\n• Price Feed → ONIX nákupné cenníky\n• Stock Feed → kontrola dostupnosti pred objednávkou\n• Media Feed → e-shop obrázky (TronShop)",
    links: [
      { label: "XD Connects Web", url: "https://www.xdconnects.com" },
      { label: "XD Connects Data Feeds", url: "https://www.xdconnects.com/data-feeds" },
